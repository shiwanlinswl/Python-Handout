...
class User(BaseModel, db.Model):
    ...
    # 当前用户所发布的新闻
    # user.news_list:当前用户的发布新闻列表
    # news.user:当前新闻是那个用户发布
    ...
    def (self, value):
        pass
    """
    # password属性的get方法，当需要获取这个属性的时候：print（user.pasword)触发的是get方法
    @property
    def password(self):
        # 不允许外界密码获取
        AttributeError("密码不允许获取")

    # password属性的set方法，当给这个属性进行赋值的时候：user.password = 122345 触发的是set方法
    @password.setter
...
class Category(BaseModel, db.Model):
    ...
    # category.news_list:当前分类下面有哪些新闻
    # news.category:当前新闻属于那个分类
