...
from flask import request, render_template, session, current_app, redirect, url_for
...
def alter_category():
    ...
    try:
        ...
        current_app.logger.error(e)
        db.session.rollback()
        return jsonify(errno=RET.DBERR, errmsg="保存分类数据异常")
...
def news_edit():
    ...
    """
       ...
       2.参数检验
           2.1 页码的数据类型判断
       3.逻辑处理
           3.1 根据News.query.filter(查询条件).order_by(新闻创建时间降序).paginate(当前页码，每一页多少条数据，False)
                查询条件1(非必须)：标题中是否包含搜索关键字key
           3.2 将查询到的新闻对象列表转换为字典列表
       4.返回值
    """
    ...
    # 3.1 3.1 根据News.query.filter(查询条件).order_by(新闻创建时间降序)
    news_list = []
    current_page = 1
    total_page = 1

    # 条件列表 默认条件：查询查询未审核&审核未通过的新闻
    filter_list = []
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR, errmsg="查询用户列表分页异常")

    # 3.2 将查询到的用户对象转换为字典列表
    news_dict_list = []
...
def news_review_detail():
    ...
    """
       1.获取参数
           1.1 news_id：新闻id， action：审核行为，reason：拒绝原因
       2.参数检验
           2.1 非空判断
           2.2 action in ["accept","reject"]
       3.逻辑处理
           3.1 根据新闻id查询对应新闻对象，新闻存在再去审核
           3.2 审核通过：将new对象的status属性行为改为：0
           3.3 审核不通过：将new对象的status属性修改为：-1，同时添加拒绝原因
           3.4 保存回数据库
       4.返回值
            登录成功
    """
...
def user_list():
    ...
    """
       ...
       2.参数检验
           2.1 页码的数据类型判断
       3.逻辑处理
           3.1 根据User.query.filter(条件：用户是非管理员用户即可)进行分页查询
           3.2 将查询到的用户对象转换为字典列表
       4.返回值
       """
    # 1.1 P：查询的页码， 默认值：1 表示查询第一页的数据
...
def user_count():
    ...
    try:
        ...
        now = time.localtime()
        # 本月的开始时间：2019-01-01
        # 下一个月的开始时间：2019-02-01
        # 每一个月第一天
        mon_begin = '%d-%02d-01' % (now.tm_year, now.tm_mon)
        # strptime：将字符串格式数据转换成时间格式数据
        mon_begin_date = datetime.strptime(mon_begin, '%Y-%m-%d')
        # 查询条件：用户的创建时间 > 本月的第一天 :代表本月新增了多少用户
    ...
    try:
        ...
        # 查询条件：用户的创建时间 > 今天的开时间今天的开始时间：2019-01-05:00:00  代表：今天新增用户人数
    ...
    今天结束时间:2019-01-05 23:50 = 开始时间 + 1天

    前一天开始时间：2019-01-05 --减去1天 = 2019-01-04
    前一天束时间:2019-01-04 23:50 = 开始时间 + 1天
    。
    。
    。
    一个月之前的开始时间：2019-01-05 --减去30天  = 2018-12-05
    一个月之前结束时间:2018-12-05 23:50 = 开始时间 + 1天
...
def admin_login():
    ...
    # GET请求；返回管理员登录的模板
    if request.method == "GET":
        # 优化:当管理员已经登录，再次访问/admin/login接口的时候，我们应该直接引导到管理员的首页
    ...
    2.参数检验
        2.1 非空判断
    3.逻辑处理
        3.1 根据username账号查询出管理员用户对象
        3.2 校验管理员用户的password
        3.3 使用session记录管理员登录信息
    4.返回值
        登录成功
    """
    # 1.1 name:管理员用户账号,password：未加密密码
